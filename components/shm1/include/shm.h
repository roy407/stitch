...
struct StitchFrame {
    ...
    // 获取拼接后图像的Y和UV指针 (相对于此结构体)
    uint8_t* get_y_data() { 
        return reinterpret_cast<uint8_t*>(this + 1); 
    }
    uint8_t* get_uv_data() { 
        return get_y_data() + (width * height); 
    }
};

// 循环缓冲区控制头部
struct CircularBufferHeader {
    uint32_t magic;              // 魔数 0xABCD1234
...
private:
    ...
    size_t calculate_frame_slot_size(int width, int height);  // 计算帧槽大小
