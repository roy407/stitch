...
void LogConsumer::printNvidiaEncoderDecoderStatus() {
    ...
    {
        LOG_WARN("Encoder stats failed: {}", nvmlErrorString(last_result));
    }
...
void LogConsumer::monitorThreads() {
    ...
    // 每6秒打印一次，打印所有线程开销比较大，所以打印间隔比较长
    if (elapsed_since_print < 6) return;
    ...
    int count = 0;
    int show_count = current_threads.size();
