...
class LogConsumer : public Consumer {
    ...
    std::vector<AVPacketProducer*> m_pro;
    std::vector<StitchConsumer*> m_con;
    void printProducer(AVPacketProducer* pro, uint64_t& prev_frame_cnt, uint64_t& prev_timestamp);
...
public:
    ...
    virtual void stop();
    virtual void run();
    void setProducer(AVPacketProducer* pro);
